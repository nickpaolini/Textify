...
function getToneInstruction(tone: number): string {
  if (tone <= 20)
    return 'Use very formal, academic language. Avoid contractions and use sophisticated vocabulary.';
  if (tone <= 40)
    return 'Use formal, professional language appropriate for business contexts.';
  if (tone <= 60)
    return 'Use balanced, clear language that is neither too formal nor too casual.';
  if (tone <= 80)
    return 'Use casual, conversational language while maintaining professionalism.';
...
function getLengthInstruction(length: string): string {
  switch (length) {
    case 'brief':
      return 'Keep the output concise and to the point. Aim for brevity while preserving key information.';
    case 'detailed':
      return 'Provide a comprehensive, detailed output with thorough explanations and examples where appropriate.';
    default:
      return 'Provide a balanced output with appropriate detail and clarity.';
  }
}
...
function buildDynamicPrompt(
  ...
  if (smartControls && transformationType !== 'markdown') {
    ...
    if (smartControls.useCustomPrompt && smartControls.customPrompt?.trim()) {
      instructions.push(
        `Additional instructions: ${smartControls.customPrompt.trim()}`
      );
    }
